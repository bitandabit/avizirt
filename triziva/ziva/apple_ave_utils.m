...
kern_return_t apple_ave_utils_get_connection(io_connect_t * conn_out) {
	...
	if (KERN_SUCCESS != ret)
	{
		printf("Failed getting master port");
		goto cleanup;
	}
	...
	if (KERN_SUCCESS != ret)
	{
		printf("Failed getting matching services");
		goto cleanup;
	}
	...
	while(IOIteratorIsValid(itr) && (service = IOIteratorNext(itr))) {
		...
		if (KERN_SUCCESS != ret)
		{
			printf("Error opening service %s", service_name);
			continue;
		}
	...
	if (0 == connection)
	{
		printf("Service %s not found!", IOKIT_SERVICE_APPLE_AVE_NAME);
		ret = KERN_ABORTED;
	}
