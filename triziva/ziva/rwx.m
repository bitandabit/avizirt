...
static kern_return_t rwx_trigger_handler()  {
	...
	{
		//printf("Error preparing ROP using %s/%s", SYSCTL_PREPARE_ROP);   //strerror(errno)
        ret = KERN_ABORTED;
		goto cleanup;
	}
	else if(0 != olds) {
		//printf("%s returned a normal size. seems like our sysctl handler wasn't installed.", SYSCTL_PREPARE_ROP);
		ret = KERN_ABORTED;
		goto cleanup;
	}
	...
	{
		//printf("Error preparing ROP using %s/%s", SYSCTL_EXECUTE_ROP); //, strerror(errno)
        ret = KERN_ABORTED;
	}
	else if (0 != olds) {
		//printf("%s returned a normal size. seems like our sysctl handler wasn't installed.", SYSCTL_EXECUTE_ROP);
		ret = KERN_ABORTED;
		goto cleanup;
	}
